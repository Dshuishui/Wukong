...
StateConstraint ==
    ...
    /\ \A i \in Server : Len(valueLog[i]) <= 2  
...
RequestVote(i, j) ==
    ...
    /\ Send([mtype         |-> RequestVoteRequest,
             ...
             msource       |-> i,
             mdest         |-> j])
    /\ UNCHANGED <<currentTerm, state, votedFor, candidateVars, leaderVars, logVars, kvsVars>>
...
ClientRequest(i, k, v) ==
    ...
    /\ LET \* 创建ValueLog条目
       ...
       IN  /\ valueLog' = [valueLog EXCEPT ![i] = newValueLog]
           ...
           /\ UNCHANGED <<messages, currentTerm, state, votedFor, candidateVars, leaderVars, 
                         commitIndex, offsetMap, elections>>
...
UpdateTerm(i, j, m) ==
    ...
    /\ state'          = [state EXCEPT ![i] = Follower]
    /\ votedFor'       = [votedFor EXCEPT ![i] = Nil]
    /\ UNCHANGED <<messages, candidateVars, leaderVars, logVars, kvsVars, elections>>
